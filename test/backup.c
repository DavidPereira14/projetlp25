...
#include <unistd.h>
#include <time.h>
void restore_backup(const char *src_dir, const char *dest_dir);
...
//mettre a jour le .backup en utilisant update_backup_log
...
// Fonction pour copier une sauvegarde existante vers un nouveau répertoire
void restore_backup(const char *src_dir, const char *dest_dir) {
    DIR *src;
    struct dirent *entry;
    struct stat src_stat;
